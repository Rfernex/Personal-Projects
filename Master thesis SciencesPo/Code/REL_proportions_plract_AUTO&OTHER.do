...
* Using the variable you defined: plur_ae_type
capture drop is_type1 is_type2 is_type3 is_type4
gen byte is_type1 = (plur_ae_type == 1) // Other + Other
gen byte is_type2 = (plur_ae_type == 2) // Other + Entrepreneur
gen byte is_type3 = (plur_ae_type == 3) // Entrepreneur + Other
gen byte is_type4 = (plur_ae_type == 4) // Entrepreneur + Entrepreneur
...
* -- Group A: Main Job is "Other" (Non-Entrepreneur) --
forvalues c = 1/2 {
    quietly svy, subpop(if autoent == 2 & acteu == 1 & annee >= 2013 & annee <= 2020): regress is_type`c' i.annee
...
* -- Group B: Main Job is Entrepreneur --
forvalues c = 3/4 {
    quietly svy, subpop(if autoent == 1 & acteu == 1 & annee >= 2013 & annee <= 2020): regress is_type`c' i.annee
    quietly margins annee
    matrix M = r(table)
...
twoway ///
    ...
    legend(order(5 "Other + Other (among Non-Ent)" 6 "Other + Ent (among Non-Ent)" ///
                 7 "Ent + Other (among Ent)" 8 "Ent + Ent (among Ent)") ///
           rows(2) position(6)) ///
    title("Evolution of Pluriactivity by Main Status (2013-2020)") ///
    ytitle("Proportion of Respective Subpopulation") ///
    xtitle("Year") ///
    xlabel(2013(1)2020) ///
    note("Source: Enquête Emploi (2013-2020). Denominators differ by category group.")
graph export "output/plract_AUTO_REL_yearly.png", replace width(2000)
...
* -- Group A: Main Job is "Other" --
forvalues c = 1/2 {
    quietly svy, subpop(if autoent == 2 & acteu == 1 & date >= `start' & date <= `end'): regress is_type`c' i.date
...
* -- Group B: Main Job is Entrepreneur --
forvalues c = 3/4 {
    quietly svy, subpop(if autoent == 1 & acteu == 1 & date >= `start' & date <= `end'): regress is_type`c' i.date
    quietly margins date
    matrix M = r(table)
...
twoway ///
    ...
    legend(order(5 "Other + Other (among Non-Ent)" 6 "Other + Ent (among Non-Ent)" ///
                 7 "Ent + Other (among Ent)" 8 "Ent + Ent (among Ent)") ///
           rows(2) position(6)) ///
    title("Evolution of Pluriactivity by Main Status (Quarterly 2013-2020)") ///
    ytitle("Proportion") ///
    xtitle("Quarter") ///
    xlabel(`start'(4)`end', format(%tq) angle(45)) ///
    note("Source: Enquête Emploi (2013-2020).")
graph export "output/plract_AUTO_REL_quarterly.png", replace width(2000)
